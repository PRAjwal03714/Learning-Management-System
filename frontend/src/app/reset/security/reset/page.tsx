...
export default function ResetPasswordPage() {
  ...
  const handleReset = async () => {
    if (!email || !token) {
      setErrorMsg('Missing credentials. Please start from the beginning.');
      return;
    }
    ...
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/reset-password`, {
        ...
        body: JSON.stringify({ email, newPassword: password, resetToken: token }),
      });
  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-gradient-to-b from-white to-blue-200 px-4">
      <div className="max-w-md w-full bg-white p-8 rounded-md shadow-lg">
        <h1 className="text-2xl font-bold text-center mb-6 text-gray-800">Reset Your Password</h1>
